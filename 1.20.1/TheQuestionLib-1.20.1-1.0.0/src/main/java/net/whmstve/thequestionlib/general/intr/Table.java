...
public interface Table<Row,Column,Value>{
    /**
    ...
    /**
     * Returns the value corresponding to the given row and column and row keys, or {@code null} if no such
     * mapping exists.
     *
     * @param rowKey key of row to search for
     * @param columnKey key of column to search for
    ...
    Value get(Row rowKey, Column columnKey);
     ...
     *     for the keys
     */
    Value put(Row rowKey,  Column columnKey,  Value value);
     ...
     *     for the keys
     */
    Value putIfAbsent(Row rowKey,  Column columnKey,  Value value);
     ...
     */
    void putAll(Table<? extends Row, ? extends Column, ? extends Value> table);
    ...
    Value remove(Row rowKey, Column columnKey);
     ...
     */
    Map<Column, Value> row( Row rowKey);
     ...
     */
    Map<Row, Value> column( Column columnKey);
     ...
     */
    Map<Row, Map<Column, Value>> rowMap();
     ...
     */
    Map<Column, Map<Row, Value>> columnMap();
     ...
     */
    default Value calculateIfAbsent(Row rowKey, Column columnKey, @NotNull BiFunction<? super Row, ? super Column, ? extends Value> mappingBiFunction){
        Value v,newV;
     ...
     */
    default Value calculateIfPresent(Row rowKey, Column columnKey, @NotNull BiFunction<? super Row, ? super Column, ? extends Value> remappingFunction){
        Value oldV,newV;
     ...
     * @return the new value or null if there is an already present value or the new value is null.
     */
    default Value calculate(Row rowKey, Column columnKey, @NotNull TriFunction<? super Row, ? super Column, ? super Value, ? extends Value> remappingFunction){
        Value oldV = get(rowKey,columnKey);
